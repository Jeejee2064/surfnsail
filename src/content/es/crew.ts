import type { CrewMember } from "@/lib/content-types";

export const crewIntro = {
  eyebrow: "Conoce al Equipo",
  heading: "Las personas detrás de cada charter",
  body: [
    "Surfnsail está formado por un equipo pequeño y unido que ama de verdad esta costa. Aquí están todas las personas que podrías encontrar a bordo.",
  ],
};

export const crew: CrewMember[] = [
  {
    slug: "marius",
    name: "Marius Swart",
    role: "Ingeniero / Capitán",
    photo: "/marius-swart.jpeg",
    bio: [
      "Hola, soy Marius Swart. Tengo más de 20 años de experiencia en la industria de la navegación y siempre me ha apasionado la vida en el mar.",
      "En 2021, mi esposa y yo fundamos Surfnsail, donde empezamos siendo toda la tripulación, recibiendo personalmente cada charter y asegurándonos de que nuestros huéspedes vivieran una experiencia inolvidable.",
      "A medida que Surfnsail ha crecido, mi rol se ha volcado más hacia el lado técnico del negocio. Me encargo del barco, asegurándome de que esté siempre mantenido, seguro y listo para cada charter. También asumo el rol de capitán cuando el capitán habitual no está disponible, ayudando a que cada viaje sea seguro, fluido y memorable.",
      "Ya sea entre bambalinas o al timón, mi objetivo es siempre el mismo: ofrecer a nuestros huéspedes una experiencia de navegación excepcional.",
    ],
  },
  {
    slug: "mireille",
    name: "Mireille Robinson",
    role: "Gerente",
    photo: "/mireille-robinson.jpeg",
    bio: [
      "Me llamo Mireille Robinson. Soy abogada de profesión, pero empecé a navegar junto a mi esposo, Marius, en Ciudad del Cabo hace 10 años, y desde que nos mudamos a Bocas del Toro en 2021, me he encargado de la parte operativa y administrativa de Surfnsail, además de llevar charters junto a él.",
      "Durante los primeros años, Marius y yo llevábamos todos los charters nosotros mismos, pero en los últimos 2 años hemos podido expandir el negocio y contratar al resto de nuestro equipo, así que en esta etapa me enfoco principalmente en gestionar las reservas y al equipo, y en asegurarme de que el bote esté siempre limpio y bien abastecido, y de que se atienda cada necesidad de nuestros huéspedes.",
    ],
  },
  {
    slug: "maga",
    name: "Alberto Maga",
    role: "Capitán, Aventura",
    photo: "/maga.webp",
    bio: [
      "Soy capitán profesional desde 2001, realizando tours por todo el archipiélago de Bocas del Toro durante más de dos décadas. Tengo experiencia operando catamaranes Fountaine Pajot y desde entonces he pasado a capitanear catamaranes Leopard, lo que me da un profundo conocimiento de la navegación en multicasco en estas aguas.",
      "He completado más de 200 tours en algunos años, navegando las islas, arrecifes y canales de Bocas del Toro en casi todas las condiciones imaginables. He trabajado extensamente como capitán profesional en la zona, construyendo una reputación de charters seguros, confiables y placenteros para huéspedes de todos los niveles de experiencia.",
      "¡Estoy deseando llevarte a ti, a tu familia y a tus amigos en un charter de navegación inolvidable por las islas de Bocas del Toro!",
    ],
  },
  {
    slug: "jeremy",
    name: "Jeremy",
    role: "Capitán, Exta Sea",
    photo: "/jeremy.jpeg",
    bio: [
      `Soy capitán profesional desde 2002, tras obtener mi licencia de la Guardia Costera de 100 toneladas ("100 ton masters"). Soy originario de California, pero viví en las Bahamas y Haití durante 28 años antes de mudarme a Panamá. He trabajado extensamente como capitán profesional y recreativo, habiendo sido dueño y vivido en numerosas embarcaciones, además de haber operado muchos barcos de forma profesional. Mi experiencia es amplia y diversa: desde trabajar como primer oficial en un mega yate de 130 pies, hasta construir mi propio velero en Sudáfrica y navegarlo por Sudáfrica y a través del Atlántico en 2004. Fui dueño de una empresa de charters en las Bahamas de 2021 a 2024, y he tenido muchos huéspedes felices navegando con el "Capitán J". ¡Estoy deseando llevarte a ti, a tu familia y a tus amigos en un charter de navegación inolvidable por las islas de Bocas del Toro!`,
    ],
  },
  {
    slug: "genna",
    name: "Genna van Niekerk",
    role: "Chef / Camarera",
    photo: "/genna.webp",
    bio: [
      "¡Hola! Soy Genna, y soy la chef y camarera a bordo desde 2024. Me encanta crear comidas deliciosas y frescas para nuestros huéspedes, mientras me aseguro de que todos se sientan relajados, cómodos y bien atendidos durante todo el día.",
      "Cuando no estoy en el agua, normalmente me encontrarás dirigiendo mis dos restaurantes en la hermosa Bocas del Toro. La hospitalidad siempre ha sido una gran parte de mi vida, pero hay algo especial en cambiar la cocina por el mar. Me encanta pasar el día rodeada de océano y naturaleza, explorar las increíbles islas, y compartir este hermoso rincón de Panamá con visitantes de todo el mundo.",
      "Lo mejor de mi trabajo es poder conocer a tantas personas maravillosas, escuchar sus historias, y ser parte de su experiencia de vacaciones. Ya sea preparando una comida memorable, sirviendo tu bebida favorita, o simplemente asegurándome de que tengas todo lo que necesitas, espero ayudar a crear un día en el agua que nunca olvidarás.",
    ],
  },
  {
    slug: "daniela",
    name: "Daniela Elizondo",
    role: "Anfitriona",
    photo: "/daniela.jpeg",
    bio: [
      "¡Hola! Soy Daniela, y me uní al equipo este año como anfitriona. Me encanta recibir a los huéspedes a bordo y ayudar a crear una experiencia divertida, relajante e inolvidable en el agua.",
      "Cuando no estoy en el mar, soy veterinaria e instructora de yoga. Ambos roles me han enseñado la importancia de la compasión, la atención plena y el cuidado de los demás — cualidades que llevo conmigo cada día a bordo.",
      "Disfruto conocer gente de todo el mundo, compartir la belleza del archipiélago, y asegurarme de que cada huésped se sienta cómodo, cuidado y verdaderamente como en casa. ¡Espero ser parte de algunos de tus recuerdos de vacaciones favoritos!",
    ],
  },
];
