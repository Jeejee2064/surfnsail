import type { CrewMember } from "@/lib/content-types";

export const crewIntro = {
  eyebrow: "Meet the Crew",
  heading: "The people behind every charter",
  body: [
    "Surfnsail is run by a small, close-knit crew who genuinely love this coastline. Here's everyone you might meet on board.",
  ],
};

export const crew: CrewMember[] = [
  {
    slug: "marius",
    name: "Marius Swart",
    role: "Engineer / Captain",
    photo: "/marius-swart.jpeg",
    bio: [
      "Hi, I'm Marius Swart. I have over 20 years of experience in the sailing industry and have always been passionate about life on the water.",
      "In 2021, my wife and I founded Surfnsail, where we started out as the sole crew, personally hosting every charter and ensuring our guests had an unforgettable experience.",
      "As Surfnsail has grown, my role has shifted more toward the technical side of the business. I take care of the vessel, making sure it is always maintained, safe, and ready for every charter. I also step in as captain whenever the regular captain is unavailable, helping ensure every trip is safe, smooth, and memorable.",
      "Whether I'm behind the scenes or at the helm, my goal is always the same: to provide our guests with an exceptional sailing experience.",
    ],
  },
  {
    slug: "mireille",
    name: "Mireille Robinson",
    role: "Manager",
    photo: "/mireille-robinson.jpeg",
    bio: [
      "My name is Mireille Robinson. I am an attorney by profession, but started sailing with my husband, Marius, in Cape Town 10 years ago, and since moving to Bocas del Toro in 2021, I have been managing the operational and administrative aspects of Surfnsail — as well as running charters with him.",
      "For the first couple of years, Marius and I ran all the charters ourselves, but in the last 2 years we have been able to expand the business and employ the rest of our crew — so I mainly focus on managing the bookings and crew at this stage, and ensure that the boat is always clean and well stocked, and that our guests' every need is catered for!",
    ],
  },
  {
    slug: "maga",
    name: "Alberto Maga",
    role: "Captain, Aventura",
    photo: "/maga.webp",
    bio: [
      "I have been a professional captain since 2001, doing tours throughout the Bocas del Toro archipelago for over two decades. I'm experienced in operating Fountaine Pajot catamarans and have since moved on to skippering Leopard catamarans, giving me a deep familiarity with multihull sailing in these waters.",
      "I've completed over 200 tours in some years, navigating the islands, reefs, and channels of Bocas del Toro in nearly every condition imaginable. I've worked extensively as a professional captain in the area, building a reputation for safe, reliable, and enjoyable charters for guests of all experience levels.",
      "I'm looking forward to taking you, your family and your friends on an unforgettable sailing charter through the Bocas del Toro islands!",
    ],
  },
  {
    slug: "jeremy",
    name: "Jeremy",
    role: "Captain, Exta Sea",
    photo: "/jeremy.jpeg",
    bio: [
      `I have been a professional captain since 2002 after receiving my 100 ton masters Coast Guard license. I'm originally from California but have lived in the Bahamas and Haiti for 28 years prior to moving to Panama. I have worked extensively as a professional and recreational captain having owned and lived on numerous boats as well as having operated many vessels in a professional capacity. My experience level is vast and diverse having worked as a 1st officer on a 130ft mega yacht all the way to having my own sailboat built in South Africa and skippering it around South Africa and across the Atlantic in 2004. I owned a charter company in the Bahamas from 2021 to 2024 and have had many happy guests sail with "Captain J". I'm looking forward to taking you, your family and your friends on an unforgettable sailing charter through the Bocas Del Toro islands!`,
    ],
  },
  {
    slug: "genna",
    name: "Genna van Niekerk",
    role: "Chef / Stew",
    photo: "/genna.webp",
    bio: [
      "Hi! I'm Genna, and I've been the Chef and Steward on board since 2024. I love creating delicious, fresh meals for our guests while making sure everyone feels relaxed, comfortable, and well looked after throughout the day.",
      "When I'm not out on the water, you'll usually find me running my two restaurants in beautiful Bocas del Toro. Hospitality has always been a huge part of my life, but there's something special about swapping the kitchen for the sea. I love spending the day surrounded by the ocean and nature, exploring the incredible islands, and sharing this beautiful corner of Panama with visitors from around the world.",
      "The best part of my job is getting to meet so many wonderful people, hearing their stories, and being part of their vacation experience. Whether I'm preparing a memorable meal, serving your favorite drink, or simply making sure you have everything you need, I hope to help create a day on the water you'll never forget.",
    ],
  },
  {
    slug: "daniela",
    name: "Daniela Elizondo",
    role: "Host",
    photo: "/daniela.jpeg",
    bio: [
      "Hi! I'm Daniela, and I joined the team this year as a host. I love welcoming guests on board and helping create a fun, relaxing, and unforgettable experience out on the water.",
      "When I'm not at sea, I'm a veterinarian and a yoga instructor. Both of these roles have taught me the importance of compassion, mindfulness, and caring for others — qualities I bring with me every day on the boat.",
      "I enjoy meeting people from all over the world, sharing the beauty of the archipelago, and making sure every guest feels comfortable, cared for, and truly at home. I look forward to being part of some of your favourite vacation memories!",
    ],
  },
];
